...
(function () {
  ...
  function showWhatsAppCTA(phone, message) {
    clearOptions().then(function () {
      ...
      /* Fallback para WebViews que bloqueiam navegação normal */
  ...
  /**
   * Corrige a altura do viewport para WebViews (Instagram, Facebook, etc.)
   * que não suportam dvh e onde 100vh inclui a barra do navegador.
   */
  function fixViewportHeight() {
    ...
    /* Instagram WebView dispara orientationchange em vez de resize às vezes */
  ...
  function init() {
    ...
    /* Fallback: se após 3s os itens ainda não estiverem visíveis, forçar */
    setTimeout(function () {
      var items = document.querySelectorAll(".animate-item");
